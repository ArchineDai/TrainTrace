...
void main() {
  ...
  Suggestion eval(List<ExercisePerformance> recent, {int lo = 10, int hi = 15, double inc = 2.5}) =>
      SuggestionEngine.evaluate(SuggestionInput(
        recent: recent,
        targetRepMin: lo,
        targetRepMax: hi,
        minIncrementKg: inc,
  ...
  test('高位下拉 20×12/12/12（10–15）→ 保持', () {
    final s = eval([perf([(20, 12, null), (20, 12, null), (20, 12, null)])]);
  ...
  test('二头弯举机 12×5 RIR0（10–15，增量 2）→ 降两档到 8kg', () {
    final s = eval([perf([(12, 5, 0)])], inc: 2);
  ...
  test('哑铃侧平举 5×8/8/8（12–20，增量 1）→ 第 1 组低于下限 3 以上，降两档到 3kg', () {
    final s = eval([perf([(5, 8, 1), (5, 8, 0), (5, 8, 0)])], lo: 12, hi: 20, inc: 1);
    expect(s.kind, SuggestionKind.decrease);
    expect(s.suggestedWeightKg, 3);
  });

  test('第 1 组略低于下限 → 只降一档', () {
    final s = eval([perf([(20, 9, null), (20, 8, null)])]);
    expect(s.kind, SuggestionKind.decrease);
    expect(s.suggestedWeightKg, 17.5);
  });

  test('第 1 组 RIR 0 即使次数达标也降重', () {
    final s = eval([perf([(20, 12, 0), (20, 11, 1)])]);
    expect(s.kind, SuggestionKind.decrease);
  ...
  test('反向蝴蝶机 12×12/6/6，第 1 组 RIR1 后两组 RIR0（12–20）→ 保持并提示后段掉次数', () {
    final s = eval([perf([(12, 12, 1), (12, 6, 0), (12, 6, 0)])], lo: 12, hi: 20);
    expect(s.kind, SuggestionKind.hold);
  ...
  test('全部达到上限、无 RIR → 加一档', () {
    final s = eval([perf([(20, 15, null), (20, 15, null), (20, 15, null)])]);
  ...
  test('全部达到上限但后段 RIR 0 → 保持，不加重', () {
    final s = eval([perf([(20, 15, 1), (20, 15, 0)])]);
    expect(s.kind, SuggestionKind.hold);
    expect(s.reason, contains('RIR 为 0'));
  });

  test('多数组在区间、个别掉出 → 保持', () {
    final s = eval([perf([(20, 13, null), (20, 11, null), (20, 9, null)])]);
    expect(s.kind, SuggestionKind.hold);
    expect(s.title, '当前重量合适，保持');
  });

  test('哑铃增量 1.0 时建议重量按 1 取整；不会低于 0', () {
    final s = eval([perf([(1, 3, 0)])], inc: 1);
    expect(s.kind, SuggestionKind.decrease);
  ...
  test('工作重量取正式组里的最大值，热身组不算', () {
    final p = ExercisePerformance(
      ...
      sets: const [
        WorkoutSet(id: 'a', workoutExerciseId: 'y', setIndex: 0, setType: SetType.warmup, weightKg: 10, reps: 15, isCompleted: true),
        WorkoutSet(id: 'b', workoutExerciseId: 'y', setIndex: 1, weightKg: 20, reps: 15, isCompleted: true),
        WorkoutSet(id: 'c', workoutExerciseId: 'y', setIndex: 2, weightKg: 22.5, reps: 15, isCompleted: true),
      ],
    );
    final s = eval([p]);
