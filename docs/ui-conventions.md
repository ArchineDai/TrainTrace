...
| 结束训练 | AppBar 右侧 | 橙色文字 `TextButton`（触控区 48），左边挨着等宽数字的用时；AppBar 单行、无实心按钮 | 训练页 |
| 放弃训练 | 首页横幅 | 训练页里没有放弃入口；一组都没完成时点「结束训练」直接问是否放弃 | 首页、训练页 |
...
两行都按 Hevy / Strong 对齐：结束在右上角，加动作在列表末尾。

2026-09-08 修订：结束训练从实心按钮改为橙色文字，用时从标题下方移到它左边，AppBar 收成一行。
页面里的填充橙已经在每组的 ✓ 和键盘「完成」上，一场只点一次的动作不该每次抬头都抢注意力；
单行也省回约 20dp 给键盘。放弃训练的「…」菜单一并去掉：退到首页横幅上有「放弃」，
没完成任何一组就点结束会直接问是否放弃（Hevy 的做法），误开的空训练不用绕路，也不再存空训练。
